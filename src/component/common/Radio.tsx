...
const Radio = ({ label, checked = false, ...props }: RadioProps) => {
  ...
  return (
    <label className="flex items-center gap-4 group" onClick={changeRadioChecked}>
      ...
      <div className="border-2 border-gray-600 relative flex justify-center items-center w-[20px] h-[20px] rounded-full before:absolute before:flex before:items-center before:w-[10px] before:h-[10px] before:rounded-full peer-checked:before:bg-violet-500 group-hover:after:w-[40px] group-hover:after:h-[40px] group-hover:after:absolute group-hover:after:bg-violet-300/50 group-hover:after:rounded-full peer-disabled:after:hidden peer-disabled:opacity-50" />
