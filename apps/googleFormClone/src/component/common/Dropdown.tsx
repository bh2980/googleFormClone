...
const Dropdown = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (selectorTop + selectorHeight + listHeight >= window.innerHeight) {
        dropdownListRef.current.style.top = `${
          window.innerHeight - listHeight - 32
        }px`;
      } else {
  ...
  return (
    <div
      className="relative w-[320px] mobile:w-full group"
      tabIndex={0}
    ...
    >
      ...
      >
        <span>
          {selectIdx === null ? '선택' : itemList[selectIdx]?.content}
        </span>
        {isOpen ? (
          <RiArrowDropUpFill className={ICON_CLASS} />
        ) : (
          <RiArrowDropDownFill className={ICON_CLASS} />
        )}
      </div>
      <ul
        ref={dropdownListRef}
